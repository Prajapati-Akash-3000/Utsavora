    ...
    <div className="w-3/4 flex flex-col h-full overflow-hidden bg-gray-50">
        ...
        <div className="bg-white border-b p-4 flex gap-4 items-center shadow-sm z-10">
             <div className="flex-1 flex gap-4">
                <input
                    className="border p-2 rounded w-1/3 focus:ring-2 focus:ring-purple-500 outline-none"
                ...
                />
                <select
                    className="border p-2 rounded w-1/4 bg-white focus:ring-2 focus:ring-purple-500 outline-none"
                    name="category"
             ...
             <div className="flex items-center gap-4">
                ...
                <button 
                    onClick={handleSave}
                    disabled={loading}
                    className="bg-purple-600 text-white px-6 py-2 rounded font-bold hover:bg-purple-700 disabled:opacity-50 transition shadow"
                >
                    {loading ? "Saving..." : "Save"}
                </button>
             </div>
        ...
        <div className="flex-1 flex overflow-hidden">
                ...
                <div className="border-b p-2 bg-gray-50 flex gap-2 overflow-x-auto">
                    {PLACEHOLDERS.map(ph => (
                        <button
                            ...
                            className="text-xs bg-white border border-gray-300 px-2 py-1 rounded hover:bg-purple-50 hover:border-purple-300 transition text-gray-600 whitespace-nowrap"
                        >
                            {`{{${ph}}}`}
                        </button>
                    ))}
            ...
            <div className="w-1/2 bg-gray-200 flex flex-col overflow-hidden relative">
                 <div className="absolute top-2 right-2 bg-black/50 text-white text-xs px-2 py-1 rounded backdrop-blur z-10">
                     Live Preview (Auto-Fit)
                 </div>
                 <div className="flex-1 overflow-hidden p-4"> {/* Changed to hidden/no-scroll since Canvas auto-fits */}
