// ─── Concert Neon — Vibrant, electric neon glow ───
...
const styles = {
  card: {
    ...
    background: "linear-gradient(160deg, #0C0A1A 0%, #1A0B2E 40%, #16082B 100%)",
    position: "relative", overflow: "hidden", boxSizing: "border-box",
  },
  glowOrb: (top, left, color) => ({
    position: "absolute", top, left, width: "200px", height: "200px",
    borderRadius: "50%", background: color, opacity: 0.15,
    filter: "blur(60px)", pointerEvents: "none",
  }),
  emoji: {
    fontSize: "clamp(36px, 7vw, 64px)", marginBottom: "clamp(6px, 1vw, 12px)",
    position: "relative", zIndex: 2, filter: "drop-shadow(0 0 20px rgba(168,85,247,0.4))",
  },
  eyebrow: {
    fontFamily: "Inter, system-ui, sans-serif",
    fontSize: "clamp(10px, 1.6vw, 14px)", fontWeight: 700,
    letterSpacing: "0.5em", color: "#A78BFA",
    textTransform: "uppercase", marginBottom: "clamp(8px, 1.5vw, 14px)",
    position: "relative", zIndex: 2,
  },
  title: {
    fontFamily: "Inter, system-ui, sans-serif",
    fontSize: "clamp(28px, 5.5vw, 54px)", fontWeight: 800,
    background: "linear-gradient(90deg, #A78BFA, #F472B6, #FB923C)",
    WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
    lineHeight: 1.15, marginBottom: "clamp(10px, 2vw, 18px)",
    position: "relative", zIndex: 2,
  },
  description: {
    fontFamily: "Inter, system-ui, sans-serif",
    fontSize: "clamp(11px, 1.8vw, 16px)", color: "#C4B5FD",
  ...
  dateText: {
    ...
    color: "#E9D5FF", letterSpacing: "0.08em",
  },
  venue: {
    fontFamily: "Inter, system-ui, sans-serif",
    fontSize: "clamp(12px, 2vw, 17px)", fontWeight: 500, color: "#DDD6FE",
    marginBottom: "clamp(4px, 0.5vw, 6px)", position: "relative", zIndex: 2,
  },
  city: {
    fontFamily: "Inter, system-ui, sans-serif",
    fontSize: "clamp(11px, 1.8vw, 15px)", color: "#8B5CF6",
...
export default function ConcertNeon({ data }) {
  const host = data?.title || data?.host || "Live Concert";
  const venue = data?.venue || "";
  const city = data?.city || "";
  const description = data?.description?.trim() || "Get ready for an unforgettable night!";
  ...
  return (
    <div style={styles.card}>
      <div style={styles.glowOrb("-10%", "-5%", "#A78BFA")} aria-hidden />
      <div style={styles.glowOrb("60%", "70%", "#F472B6")} aria-hidden />

      <div style={styles.emoji}>🎵</div>
      <span style={styles.eyebrow}>Live Concert</span>
